...
void write_inode_table(int fd)
{
	...
	struct ext2_inode hello_world_inode = {7};
	root_dir_inode.i_mode = EXT2_S_IFREG | EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IRGRP | EXT2_S_IROTH;
	...
	root_dir_inode.i_mtime = current_time;
	root_dir_inode.i_dtime = 0;
	root_dir_inode.i_gid = 1000;
	root_dir_inode.i_links_count = 1;
	root_dir_inode.i_blocks = 2; /* These are oddly 512 blocks */
	...
	root_dir_inode.i_blocks = 2; /* These are oddly 512 blocks */
	root_dir_inode.i_block[0] = HELLO_WORLD_FILE_BLOCKNO;
	write_inode(fd, HELLO_WORLD_INO, &hello_world_symlink_inode);
}
